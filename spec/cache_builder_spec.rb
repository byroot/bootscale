...
RSpec.describe Bootscale::CacheBuilder do
  it "list all the files in the provided load paths" do
    ...
    expect(cache.keys.sort).to eq %w(application_controller.rb application_helper.rb concerns/requireable.rb)
  end
  ...
  it "doesn't break if elements of the load path are Pathname instances" do
    ...
    expect(cache.keys.sort).to eq %w(application_controller.rb concerns/requireable.rb)
  end
  ...
  it "marks relative paths since they do not work normally or when switching directories" do
    ...
    expect(cache).to eq "application_controller.rb" => false, "concerns/requireable.rb" => false
  end
end
